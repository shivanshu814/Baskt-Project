...
export default function Faucet() {
  ...
  const { balance, loading: balanceLoading, error: balanceError, refetch: refetchUSDCBalance } = useUSDCBalance();
  ...
  return (
    <div className="rounded-xl p-6 bg-white/5 border border-white/10 flex flex-col items-center gap-4">
      ...
      <div className="text-white/70">USDC Balance: <span className="font-mono">{balanceLoading ? 'Loading...' : balance}</span></div>
      {balanceError && <div className="text-red-400">{balanceError}</div>}
      {success && <div className="text-green-400">{success}</div>}
      <Button size="lg" className="mt-2 w-full text-lg font-bold py-4" onClick={handleMint} disabled={balanceLoading}>
