...
import { ActionCardProps } from '../../../types/vault';
          ...
          <div className="space-y-4 p-0">
            <div className="relative">
              <Input
                ...
                className="pr-20 text-md font-semibold bg-foreground/5 border border-border focus:border-primary rounded-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                disabled={loading}
              />
            ...
            <div className="space-y-2 text-sm bg-foreground/5 p-3 rounded-sm">
              {expectedOutput && (
                <div className="flex justify-between text-muted-foreground">
                  <span>You will receive:</span>
                  <span className="font-medium">{expectedOutput} BLP</span>
                </div>
              )}
              {fee && (
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <div className="border-t border-border/50 pt-2 mt-2">
                    {queueInfo.estimatedWaitTime && (
                      <div className="flex justify-between text-muted-foreground">
                        ...
                        <span className="font-medium">
                          {queueInfo.estimatedWaitTime.toFixed(1)}h
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between text-muted-foreground">
                      <span>Total in Queue:</span>
                      <span className="font-medium">{queueInfo.totalQueueItems}</span>
                    </div>
                  </div>
                </>
              )}
            ...
            <Button
              className={`w-full text-base font-bold py-3 rounded-sm shadow-md transition-all duration-200 ${
                color === 'green'
