...
import { throttledLog } from './log-throttle';
...
import { STREAM_SCHEMA_MAP } from './schema-map';
...
export class DataBus extends EventEmitter {
  ...
  async publish<T>(stream: StreamName, payload: T): Promise<string> {
    // Runtime schema validation before anything else
    const schema = STREAM_SCHEMA_MAP[stream as StreamName];
    if (schema) {
      const parse = (schema as any).safeParse(payload);
      if (!parse.success) {
        messageRejectedCount.inc({ stream, reason: 'validation_failed' });
        throw new Error(`Payload validation failed for stream ${stream}: ${parse.error.message}`);
      }
    }
    // Check payload size
    ...
    args.push('*', 'data', JSON.stringify(envelope));
          ...
          for (const [streamName, streamMessages] of messages) {
            for (const [id, fields] of streamMessages) {
              ...
              try {
                ...
                logger.error('JSON parse error', { err, id, stream });
    ...
    const monitor = async () => {
      while (!signal.aborted) {
        try {
          ...
          const info = await this.redis.xinfo('STREAM', config.stream);
          if (Array.isArray(info) && info.length > 1 && Number.isFinite(Number(info[1]))) {
            streamLengthGauge.set({ stream: config.stream }, Number(info[1]));
          } else {
            logger.warn('Unexpected XINFO format', { stream: config.stream, info });
          }
        } catch (err) {
...
export { STREAMS, getAllStreamValues } from './streams';
