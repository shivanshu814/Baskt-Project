...
impl LiquidityPool {
    ...
    pub fn calculate_shares_to_mint(&self, gross_deposit_amount: u64, fee_amount: u64) -> Result<u64> {
        // Ensure the deposit meets the minimum requirement
        require!(gross_deposit_amount >= self.min_deposit, PerpetualsError::BelowMinimumDeposit);
    ...
    pub fn calculate_withdrawal_amount(&self, lp_tokens_to_burn: u64) -> Result<u64> {
        require!(self.total_shares > 0, PerpetualsError::InsufficientLiquidity);
    ...
    pub fn process_withdrawal(&mut self, lp_tokens_to_burn: u64, withdrawal_amount: u64) -> Result<()> {
        // Update pool state
        self.total_liquidity = self.total_liquidity
