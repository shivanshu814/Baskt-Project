...
pub struct GetAssetPrice<'info> {
    ...
    /// CHECK: This is checked by the constraint
    #[account(constraint = oracle.key() == asset.oracle.oracle_account.key() @ PerpetualsError::InvalidOracleAccount)]
...
pub fn get_baskt_nav(ctx: Context<GetBasktNav>) -> Result<u64> {
    ...
    let current_prices =
        Baskt::process_asset_oracle_pairs(remaining, clock.unix_timestamp, Some(baskt))?;
