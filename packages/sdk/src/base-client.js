...
export class BaseClient {
     ...
     */
    constructor(connection, provider, userPublicKey) {
        // Default values
        ...
        this.program = new anchor.Program(BasktV1Idl);
        this.connection = connection;
        this.provider = provider;
        // Initialize helpers
        this.oracleHelper = new OracleHelper(this.program, userPublicKey || this.getPublicKey(), this.provider);
        // Derive protocol PDA
        [this.protocolPDA] = PublicKey.findProgramAddressSync([Buffer.from('protocol')], this.program.programId);
    }
    /**
     ...
     */
    async createCustomOracle(protocol, oracleName, price = this.DEFAULT_PRICE, exponent = this.DEFAULT_PRICE_EXPONENT, confidence, ema, timestamp) {
        ...
        if (this.lookupTable) {
            postInstructions.push(await extendLookupTable(this.connection, this.lookupTable, this.getPublicKey(), this.getPublicKey(), [oracle]));
        }
        const result = await this.oracleHelper.createCustomOracle(protocol, oracleName, rawPrice, exponent, emaBN, rawConfidence, timestamp, postInstructions);
        return result;
    }
    /**
     ...
     */
    createOracleParams(oracleAddress, oracleType, maxPriceError = this.DEFAULT_PRICE_ERROR, maxPriceAgeSec = this.DEFAULT_PRICE_AGE_SEC, priceFeedId = '') {
     ...
     * @param confidence Confidence interval (optional)
     */
    async updateOraclePrice(oracleName, oracleAddress, price, exponent = this.DEFAULT_PRICE_EXPONENT, ema, confidence) {
        // Convert to raw price with exponent if a number is provided
        const rawPrice = typeof price === 'number' ? new anchor.BN(price) : price;
        // Default confidence to 1% of price if not specified
        const rawConfidence = confidence ?? (typeof rawPrice === 'number' ? rawPrice / 100 : undefined);
        await this.oracleHelper.updateCustomOraclePrice(oracleName, oracleAddress, rawPrice, exponent, ema ?? rawPrice, rawConfidence);
    }
    /**
     ...
     */
    createAssetParams(ticker, oracleParams, targetPrice, maxSupply, collateralRatio = 15000, // 150% by default
    ...
    permissions = { allowLongs: true, allowShorts: true }) {
    ...
    async initializeLookupTable() {
        if (this.lookupTable) {
            return undefined;
        }
        const { lookupTableAddress, createInstruction } = await createLookupTableInstructions(this.connection, this.getPublicKey(), this.getPublicKey());
    ...
    async hasRole(account, role) {
        ...
        return protocol.accessControl.entries.some((entry) => entry.account.toString() === account.toString() &&
     ...
     */
    async hasPermission(account, role) {
        const protocol = await this.getProtocolAccount();
    ...
    async addAsset(params) {
        // Find the asset PDA
        const [assetAddress] = await PublicKey.findProgramAddress([Buffer.from('asset'), Buffer.from(params.ticker)], this.program.programId);
        const postInstructions = [];
        // Add the asset to the lookup table
        if (this.lookupTable) {
            postInstructions.push(await extendLookupTable(this.connection, this.lookupTable, this.getPublicKey(), this.getPublicKey(), [assetAddress]));
        }
     ...
     */
    async addAssetWithPythOracle(ticker, pythAddress, permissions = { allowLongs: true, allowShorts: true }, priceError = 100, priceAgeSec = 60) {
     ...
     */
    async getAssetRaw(assetPublicKey) {
     ...
     */
    async createBaskt(basktName, assetConfigs, isPublic, assetOraclePairs) {
        // Derive the baskt PDA
        const [basktId] = PublicKey.findProgramAddressSync([Buffer.from('baskt'), Buffer.from(basktName)], this.program.programId);
    ...
    async sendAndConfirm(instructions) {
    ...
    async getVersionTransaction(instructions) {
        // Get the latest blockhash
        const { blockhash } = await this.connection.getLatestBlockhash();
        const lookupTables = [];
        if (this.lookupTable) {
            const lookupTableAccount = (await this.connection.getAddressLookupTable(new PublicKey(this.lookupTable))).value;
     ...
     */
    async getAssetPrice(assetAddress, oracleAddress) {
        try {
    ...
    async updateFeatureFlags(featureFlags) {
        try {
            const tx = await this.program.methods
                .updateFeatureFlags(featureFlags.allowAddLiquidity, featureFlags.allowRemoveLiquidity, featureFlags.allowOpenPosition, featureFlags.allowClosePosition, featureFlags.allowPnlWithdrawal, featureFlags.allowCollateralWithdrawal, featureFlags.allowBasktCreation, featureFlags.allowBasktUpdate, featureFlags.allowTrading, featureFlags.allowLiquidations)
                .accounts({
                owner: this.getPublicKey(),
                protocol: this.protocolPDA,
            })
     ...
     */
    async getBasktNav(basktAddress, assetOraclePairs = []) {
     ...
     * @returns Transaction signature
     */
    async rebalanceBaskt(basktId, assetConfigs, assetOraclePairs) {
        ...
        const assetParams = assetConfigs.map((config) => {
            ...
            const existingConfig = baskt.currentAssetConfigs.find((ac) => ac.assetId.toString() === config.assetId.toString());
            if (!existingConfig) {
